...
// The lecture demo expressed that as:
// let kitten1 = {
//     name: 'Frankie',
//   }
...
//const is used instead of let because that timeSlot is only ever mean to equal this array



//The code below is defining the following keys for Seattle:  city, min, max, avgCookies, sales, and estimate
...
// and from that we can calculate and store the simulated number of cookies purchased during each hour (which is estimate)
...
const seattle = {
    ...
    avgCookies: 6.3,
    sales: [],
    //The estimated sales per hour is going to be in the array that gets displayed in the column next to the displayed hour (which is in the array called timeSlot). Continuing to follow the example of using let object = {key: value} results in
    estimate:  function(){
    this.sales = estimateSales(this);}
    //which we can call on later with seattle.estimate
}
...
//we can use either an <article> or a <section> per cookie stand.  So let's use article
